...
/*
 * ElementTransferHandler.java
 *
 * Created on June 5, 2005, 12:50 AM
 *
...
/**
 * Implements custom import behavior for the Elements tabs
 * @see VariableTransferHandler
 */
public class ElementTransferHandler extends VariableTransferHandler{
	
	/** Creates a new instance of ElementTransferHandler */
		...
		final Tab tab = table.getTab();
			...
			for(int i = 0; i < elts.length; i++){
				...
				if(GiANT.gui.kash.isElement(eltName)){
					tab.addNewVariable(eltName);
				}else{
					GiANT.gui.appendConsoleText("\nElements accepts only Elements from a drop.", true);
